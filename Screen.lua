...
--[[
  Lưu ý:
  • Sử dụng task.wait, task.spawn thay cho wait, spawn để tối ưu.
  • Các tham số hiệu ứng được cấu hình trong bảng "config".
  • Một số hàm được cập nhật để giảm tải RenderStepped.
]]
...
local config = {
	...
	Shadow = {
		...
		Smoothing = 0.2,  -- Hệ số nội suy chuyển động bóng
		Layers = {
...
local function updateAdvancedShadows()
	if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
		...
		-- Dự đoán vị trí bóng bằng cách chiếu tia từ vị trí nhân vật theo hướng ngược Mặt Trời
		local rayOrigin = hrp.Position
		local rayDirection = -sunDir * 100
		local rayParams = RaycastParams.new()
		rayParams.FilterDescendantsInstances = {player.Character, unpack(shadowLayers)}
		...
		-- Điều chỉnh kích thước bóng theo góc Mặt Trời
		...
		-- Tính toán hướng bóng theo mặt đất (dựa trên groundNormal)
		...
		-- Nội suy chuyển động bóng để mượt mà hơn
...
-- Nâng cấp hiệu ứng bóng: Thêm raycasting để xác định vị trí chính xác trên mặt đất,
-- căn chỉnh theo độ nghiêng của mặt đất và áp dụng nội suy chuyển động bóng mượt mà.
